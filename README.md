...
1. Технолог создаёт производственную задачу.
2. Для задачи указываются производственный участок, приоритет, ответственный и плановые сроки.
3. Пользователь может посмотреть все задачи или отфильтровать их по статусу и приоритету.
4. Когда работа начинается, статус задачи меняется с `NEW` на `IN_PROGRESS`.
5. После завершения работы статус задачи меняется на `DONE`.
6. Система помогает отслеживать текущую загрузку, приоритетные задачи и состояние выполнения работ.
